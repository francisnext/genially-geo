...
export async function fetchGscSiteWideCtr(params: {
  ...
  const request = async (body: Record<string, unknown>) => {
    ...
    const json = (await res.json()) as {
      rows?: Array<{ keys?: string[]; clicks?: number; impressions?: number; ctr?: number }>;
      error?: { message?: string };
    };
    if (!res.ok) throw new Error(json.error?.message ?? "Search Console searchAnalytics query failed");
    return json.rows ?? [];
  ...
  // Primary path: global aggregate in one row (no dimensions).
  const aggregateRows = await request({
    startDate: params.startDate,
    endDate: params.endDate,
    dataState: "final",
  });
  ...
  if (aggregate) {
    const clicks = typeof aggregate.clicks === "number" ? aggregate.clicks : 0;
    const impressions = typeof aggregate.impressions === "number" ? aggregate.impressions : 0;
    // GSC API returns ctr as decimal (0-1); convert to percentage (0-100)
    const ctrRaw = typeof aggregate.ctr === "number" ? aggregate.ctr : impressions > 0 ? clicks / impressions : 0;
    const ctr = Math.round(ctrRaw * 10000) / 100; // e.g. 2.34 for 2.34%
    return { clicks, impressions, ctr };
  }

  // Fallback: sum per-day rows when the aggregate row is not returned.
  const byDayRows = await request({
    startDate: params.startDate,
    endDate: params.endDate,
    dimensions: ["date"],
    rowLimit: 1000,
  ...
  const ctr = Math.round((impressions > 0 ? (clicks / impressions) * 100 : 0) * 100) / 100;
  return { clicks, impressions, ctr };
}
