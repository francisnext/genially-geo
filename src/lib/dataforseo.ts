...
export async function fetchKeywordIdeas(
  ...
  const safeLocationCode = Number.isFinite(locationCode) ? Math.floor(locationCode) : 2840;
  const safeLanguageCode = (languageCode || "en").trim().toLowerCase() || "en";

  const res = await createAuthenticatedFetch()(`${API_BASE}/v3/dataforseo_labs/google/keyword_ideas/live`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify([
      {
        keywords: [q],
        location_code: safeLocationCode,
  ...
  if (!res.ok || response.status_code !== 20000) {
    throw new Error(
      response.status_message ??
        `DataForSEO error ${response.status_code ?? res.status ?? "unknown"}`
    );
  }
  ...
  const out: KeywordResearchItem[] = [];
  for (const item of rawItems) {
    if (item && typeof item === "object") {
      out.push(mapItem(item as Record<string, unknown>));
    }
  }
  return out;
}
