...
import { SampleDatasetItem } from '@/data/geo/sample-dataset';
...
export async function fetchStrategicReportHistory(maxReports = 10): Promise<StrategicReport[]> {
    const reportsCollection = collection(db, 'strategic_reports');
    const q = query(reportsCollection, orderBy('generatedAt', 'desc'), limit(maxReports));
    const snapshot = await getDocs(q);

    if (snapshot.empty) return [];
    return snapshot.docs
        .map(doc => ({ _id: doc.id, ...doc.data() } as StrategicReport))
