...
export async function GET(request: NextRequest) {
  ...
  async function tryApplySearchConsole(payload: DomainOverviewApiResponse, skipDataforseo = false): Promise<string | undefined> {
    if (!gscSiteUrl) return undefined;
    try {
      const refreshToken = await getGa4RefreshToken();
      if (!refreshToken) {
  ...
  try {
    ...
    let dataforseoKeywordsWarning: string | undefined;
    ...
    if (granularity === "daily" && !refresh && cachedMonthly && cachedMonthly.keywordCount !== undefined) {
      ...
      const [kwResult, visResult, ranked] = await Promise.all([
        fetchKeywordsForSite(domain, locationCode).catch((err) => {
          dataforseoKeywordsWarning =
            err instanceof Error ? err.message : "Failed to load DataForSEO keyword summary";
          return {
            keywordCount: 0,
            totalSearchVolume: 0,
            topKeywords: [],
    ...
    const payload: DomainOverviewApiResponse = {
      ...
      history,
      historyError:
        [visibilityResult.historyError, ga4TrafficError, dataforseoKeywordsWarning ? `DataForSEO keywords: ${dataforseoKeywordsWarning}` : undefined]
          .filter(Boolean)
          .join(" ") || undefined,
