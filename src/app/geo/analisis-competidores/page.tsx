...
import { getSampleDataset, SampleDatasetItem, getUniqueKeywords } from "@/data/geo/sample-dataset"
import { normalizeBrandName, isDirectCompetitor, DIRECT_COMPETITORS } from "@/lib/geo/competitors"
                      ...
                      <BarChart data={displayBrands.slice(0, 8)} layout="vertical">
                        ...
                        <Bar dataKey="sov" radius={[0, 4, 4, 0]} barSize={12}>
                          {displayBrands.map((entry, index) => (
                            <Cell key={index} fill={entry.name === "Genially" ? "#6c29ff" : "#cbd5e1"} />
                          ))}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                        ...
                        <Scatter name="Marcas" data={displayBrands} fill="#8884d8">
                          {displayBrands.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={entry.name === 'Genially' ? '#6c29ff' : entry.isDirect ? '#ef713f' : '#c7c3d3'} />
                          ))}
                        </Scatter>
                      </ScatterChart>
                    </ResponsiveContainer>
