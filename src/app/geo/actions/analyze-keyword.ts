...
import { saveQueryToFirestore } from '@/lib/geo/firestore-service';
...
import { DEFAULT_KEYWORDS } from '@/lib/geo/llm/config';
...
export async function analyzeKeyword(keyword: string, prompt?: string, batchId?: string, skipSave: boolean = false) {
    ...
    try {
        ...
        revalidatePath(`/geo/cluster/${encodeURIComponent(keyword)}`);
        revalidatePath('/');
