...
import SidebarMenu from "@/components/geo/SidebarMenu"
...
import { generateStrategicReportAction, getLatestStrategicReportAction, getStrategicReportHistoryAction } from "@/app/geo/actions/strategic-analysis"
import { sendSlackManualAction } from "@/app/geo/actions/send-slack-manual"
...
export default function DebilidadesOportunidadesPage() {
  ...
  const [history, setHistory] = useState<StrategicReport[]>([])
  ...
  const loadData = async () => {
    ...
    const [latestRes, historyRes] = await Promise.all([
      getLatestStrategicReportAction(),
      getStrategicReportHistoryAction()
    ])
    if (latestRes.success && latestRes.report) setReport(latestRes.report)
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--background)", paddingLeft: "16rem" }}>
        <SidebarMenu />
        <main className="flex-1 p-6 space-y-6 overflow-y-auto">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            {!report && !isGenerating ? (
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">{/* report is non-null in this branch */}
                ...
                {/* Evolución Temporal */}
                {history.length > 1 && (() => {
                  const allClusters = Array.from(new Set(history.flatMap(r => r.reportContent.clusters.map(c => c.name))))
                  const sovData = history.map(r => {
                    const point: Record<string, any> = { date: new Date(r.generatedAt).toLocaleDateString("es-ES", { day: "2-digit", month: "short" }) }
                    r.reportContent.clusters.forEach(c => { point[c.name] = parseFloat(c.sov.toFixed(1)) })
                    return point
                  })
                  const sentimentData = history.map(r => {
                    const point: Record<string, any> = { date: new Date(r.generatedAt).toLocaleDateString("es-ES", { day: "2-digit", month: "short" }) }
                    r.reportContent.clusters.forEach(c => { point[c.name] = parseFloat((c.sentiment * 10).toFixed(1)) })
                    return point
                  })
                  return (
                    <section className="space-y-6 pt-4 pb-10">
                      <header className="flex items-center gap-2 px-2">
                        ...
                        <h3 className="text-lg font-black text-slate-800">Evolución Temporal</h3>
                        <Badge variant="outline" className="border-indigo-200 text-indigo-700 font-black ml-auto">
                          {history.length} INFORMES
                        </Badge>
                      </header>

                      <Card className="border-none shadow-sm bg-white p-6">
                        <h4 className="text-sm font-black text-slate-500 uppercase tracking-wider mb-6">Share of Voice (%) por Cluster</h4>
                        <ResponsiveContainer width="100%" height={280}>
                          <LineChart data={sovData}>
                            ...
                            <Tooltip formatter={(v: any) => { const n = Number(v); return [n % 1 === 0 ? n.toString() : n.toFixed(1), ""] }} contentStyle={{ borderRadius: 8, border: "none", boxShadow: "0 4px 24px #0001" }} />
                            <Legend wrapperStyle={{ fontSize: 11, fontWeight: 700 }} />
                            {allClusters.map((name, i) => (
                              <Line key={name} type="monotone" dataKey={name} stroke={CLUSTER_COLORS[i % CLUSTER_COLORS.length]} strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} connectNulls />
                            ))}
                          </LineChart>
                        </ResponsiveContainer>
                      </Card>
                      ...
                      <Card className="border-none shadow-sm bg-white p-6">
                        <h4 className="text-sm font-black text-slate-500 uppercase tracking-wider mb-6">Sentimiento IA (/10) por Cluster</h4>
                        <ResponsiveContainer width="100%" height={280}>
                          <LineChart data={sentimentData}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                            <XAxis dataKey="date" tick={{ fontSize: 11, fontWeight: 600, fill: "#94a3b8" }} />
                            <YAxis tick={{ fontSize: 11, fill: "#94a3b8" }} domain={[-10, 10]} />
                            <Tooltip contentStyle={{ borderRadius: 8, border: "none", boxShadow: "0 4px 24px #0001" }} />
                            <Legend wrapperStyle={{ fontSize: 11, fontWeight: 700 }} />
                            {allClusters.map((name, i) => (
                              <Line key={name} type="monotone" dataKey={name} stroke={CLUSTER_COLORS[i % CLUSTER_COLORS.length]} strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} connectNulls />
                            ))}
